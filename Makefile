...
wheel_manylinux32 wheel_manylinux64 wheel_manylinuxaarch64: dist/$(PACKAGENAME)-$(VERSION).tar.gz
	...
	time docker run --rm -t \
		...
		$(if $(patsubst %32,,$@),$(MANYLINUX_IMAGE_X86_64),$(MANYLINUX_IMAGE_686),$(MANYLINUX_IMAGE_aarch64)) \
		bash -c '\
